...
def get_hotel_ratings_dict():
	...
	for line in open("../train/Hotel.csv", "r"):
		try:
			line = line.strip()
			hotel_id, rating = line.split(",")[0], float(line.split(",")[5])
...
def process():
	...
	for k, v in hotel_ratings_dict.items():
		...
		if segment not in classified_dict:
			classified_dict[segment] = {}
			classified_dict[segment][rating] = 1
		else:
			if rating not in classified_dict[segment]:
				classified_dict[segment][rating] = 1
			else:
				classified_dict[segment][rating] += 1	
